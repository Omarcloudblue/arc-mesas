// Bot del canal: mantiene el mensaje fijo de eventos, avisa de la Matriarcuda y publica
// las solicitudes nuevas del tablón. Lo llama el cron de Supabase cada minuto:
//   https://<tu-sitio>.pages.dev/api/bot?k=<BOT_KEY>
import { fetchSchedule, embedEvents, es, mp, ts, emo, REGION_ES, COLOR, site } from "../../shared/events.js";
import { loadState, saveState, supaGet } from "../../shared/state.js";
...
export async function onRequest({ request, env }) {
  ...
  if (!env.BOT_KEY || url.searchParams.get("k") !== env.BOT_KEY) return new Response("no", { status: 401 });
  const hook = env.DISCORD_WEBHOOK_URL;
  if (!hook) return new Response("falta DISCORD_WEBHOOK_URL");
  ...
  let evs;
  try { evs = await fetchSchedule(region); } catch (e) { return new Response("upstream: " + e.message); }
  const now = Date.now();
  ...
  const alerta = (embed2) => post(hook, "POST", { content: mention, embeds: [embed2], allowed_mentions: { parse: ["roles", "everyone"] } });
  let sent = 0;
  ...
  return new Response(`ok · estado ${stateOk ? "sí" : "no"} · msg ${msgId || "-"} · activos ${activos} · avisos ${sent} · tablón ${tablon}${cmd}${note}`);
}

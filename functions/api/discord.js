// Comandos slash de Discord (endpoint de interacciones). Cloudflare Pages Function.
// En el portal de Discord → General Information → Interactions Endpoint URL:
//   https://<tu-sitio>.pages.dev/api/discord
import { fetchSchedule, embedEvents, embedFocus, REGIONS, COLOR, site } from "../../shared/events.js";
import { nm, MATERIALS, norm, computeMissing, BP, bpStats } from "../../shared/data.js";
import { SUPA_URL, SUPA_KEY } from "../../shared/state.js";
...
async function verify(request, body, publicKey) {
  ...
  for (const alg of ["Ed25519", "NODE-ED25519"]) {
    try {
      const key = await crypto.subtle.importKey("raw", hex(publicKey), { name: alg, namedCurve: alg }, false, ["verify"]);
      return await crypto.subtle.verify({ name: alg }, key, hex(sig), new TextEncoder().encode(ts + body));
    } catch (e) { /* probar el siguiente nombre de algoritmo */ }
  }
  return false;
}
...
const squad = () => supa("progress?shared=eq.true&select=user_id,display_name,discord_id,avatar_url,data,blueprints,updated_at&order=updated_at.desc");
...
async function run(i, env) {
  ...
  const region = () => { const r = optVal(i, "region"); return REGIONS.includes(r) ? r : (env.EVENTS_REGION || "north-america"); };
  const memberField = (m, limit) => {
  ...
  if (name === "eventos") { const reg = region(); return embedEvents(await fetchSchedule(reg), reg, Date.now(), { groups: 3, site: SITE }); }
  if (name === "matriarcuda") { const reg = region(); return embedFocus(await fetchSchedule(reg), reg, "Matriarch"); }

  if (name === "tablon") {
    let rows = [];
    try { rows = await supa("posts?select=kind,item,qty,note,display_name,created_at&order=created_at.desc&limit=20"); }
    catch (e) { return { color: COLOR.squad, title: "Tablón", description: "Todavía no está activado." }; }
  ...
  if (name === "quien") {
    ...
    return {
      color: need.length ? COLOR.hot : COLOR.live, title: nm(key),
      thumbnail: { url: `${SITE}/icons/${key.toLowerCase().replace(/\s+/g, "-")}.png` },
  ...
  if (name === "planos") {
    ...
    if (plano) {
      const bpr = BP.find(([s]) => s === plano) || BP.find(([, n]) => norm(n) === norm(plano));
      if (!bpr) return { color: COLOR.squad, title: "Plano desconocido", description: `No encuentro «${plano}».` };
      const tiene = members.filter((m) => (m.blueprints || {})[bpr[0]]).map((m) => m.display_name || "Raider");
      const falta = members.filter((m) => !(m.blueprints || {})[bpr[0]]).map((m) => m.display_name || "Raider");
      return {
        color: tiene.length ? COLOR.live : COLOR.hot, title: bpr[1],
        thumbnail: { url: `${SITE}/icons/bp/${bpr[0]}.webp` },
        fields: [
    ...
    if (uid) {
      ...
      const ru = (i.data.resolved && i.data.resolved.users && i.data.resolved.users[uid]) || {};
      const who = ru.global_name || ru.username || "Ese raider";
      if (!m) return { color: COLOR.squad, title: who, description: "No está compartiendo su progreso." };
    ...
    return {
      color: COLOR.squad, title: "Escuadrón — planos", url: SITE,
